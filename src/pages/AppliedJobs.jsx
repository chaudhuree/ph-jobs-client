...
export default function AppliedJobs() {
  ...
  return (
    <div className="mx-auto container px-5">
        ...
        <div className="flex gap-4 items-center">
          ...
          <div className="flex gap-2 items-center ">
            <input
              ...
              value="card"
            />
            <label for="card" className="text-lg font-bold">
              Card
            </label>
          </div>

          <div className="flex justify-center md:justify-end items-center w-full  py-5 dark:text-gray-300">
            <PDFDownloadLink
              document={<PdfFile jobsData={jobs?.data?.appliedJobs} />}
              fileName="applied-jobs.pdf"
            >
              {({ loading }) =>
                loading ? (
                  <MdOutlineDownloading className="text-2xl text-amber-500 ml-4 " />
                ) : (
                  <AiOutlineDownload
                    title="download"
                    className="text-2xl text-primary ml-4 "
                  />
                )
              }
            </PDFDownloadLink>
      ...
      {view === "table" ? (
        <TableWithoutPagination jobsData={jobs?.data?.appliedJobs} />
      ) : (
        <AppliedJobsCardView jobsData={jobs?.data?.appliedJobs} />
      )}
    </div>
  );
}
